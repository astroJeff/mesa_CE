...
! <<<<<<< donor routines
      ...
      logical function check_CE(s, prev_mass_transfer_rate)
         type (star_info), pointer :: s
         real(dp), intent(in) :: prev_mass_transfer_rate
         real(dp) :: mdot, R_accretor
         include 'formats.inc'

         mdot = prev_mass_transfer_rate

         R_accretor = accretor_mass_radius_relation(m1)
         rl2 = rlobe_accretor(m1, m2, binary_separation)

         if( dabs(mdot) > 1.0d0*m2*dsqrt(standard_cgrav * s% rho(1))) then
            check_CE = .true.
         else if (r + R_accretor > binary_separation) then
            check_CE = .true.
         else
            check_CE = .false.
         endif
      ...
      logical function check_merger(s)
         ...
         if (R_accretor + R_donor .gt. binary_separation) then
            check_merger = .true.
         else
            check_merger = .false.
         endif
      ...
      real(dp) function calc_naive_A_f(s)
         ...
         E_orb_init = alpha_CE * standard_cgrav * companion_mass*Msun * s% mstar / (2.0d0 * binary_separation)
         af_E_orb_f = alpha_CE * standard_cgrav * companion_mass*Msun * s% he_core_mass*Msun / 2.0d0
